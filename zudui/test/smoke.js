...
async function main() {
  ...
  await test("isEmptyDir tolerates allowed-noise entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-empty-"));
    try {
  ...
  await test("isEmptyDir rejects node_modules and project artifacts", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-nonempty-"));
    try {
  ...
  await test("isEmptyDir rejects user files like README.md", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-userfile-"));
    try {
  ...
  await test("writeStartScripts produces sh + cmd with right content", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-scripts-"));
    try {
  ...
  await test("writeStartScripts rejects unknown framework", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-bad-fw-"));
    try {
      assert.throws(
  ...
  await test("init writes the full team layout under a stubbed broker", async () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-init-"));
    try {
      ...
      for (const role of ["pm", "dev", "reviewer", "support"]) {
        ...
        assert.ok(
          fs.existsSync(path.join(dir, role, ".zudui", "credentials.json")),
          `${role}/.zudui/credentials.json missing`,
        );
        assert.ok(
          fs.existsSync(path.join(dir, role, ".zudui", "inbox.js")),
          `${role}/.zudui/inbox.js missing`,
        );
        assert.ok(fs.existsSync(path.join(dir, `start-${role}.sh`)));
        assert.ok(fs.existsSync(path.join(dir, `start-${role}.cmd`)));

        if (process.platform !== "win32") {
          const mode = fs.statSync(path.join(dir, role, ".zudui", "credentials.json")).mode & 0o777;
          assert.equal(mode, 0o600, `${role} credentials must be 0600, got ${mode.toString(8)}`);
        }

        const creds = JSON.parse(
          fs.readFileSync(path.join(dir, role, ".zudui", "credentials.json"), "utf8"),
        );
      ...
      const team = JSON.parse(
        fs.readFileSync(path.join(dir, ".zudui", "team.json"), "utf8"),
      );
  ...
  await test("init aborts when broker rejects the first agent and writes partial marker", async () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zudui-test-init-fail-"));
    try {
      const code = await runInit(dir);
